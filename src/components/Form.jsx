...
const Form = ({ setPacientes, pacientes, paciente, setPaciente }) => {
  ...
  const handleSubmit = (e) => {
    ...
    if(paciente.id){
      ...
      // setPacientes(objetoPaciente) //mala practica porque se declaro un [] en el App.jsx
      setPacientes([...pacientes, objetoPaciente]) // buena practica
    }
  ...
  return (
    <div className='md:w-1/2 lg:w-2/5 mx-5'>
      ...
      <form className='bg-white shadow-md rounded-lg py-10 px-5 mb-10' onSubmit={handleSubmit}>
        ...
        <input type="submit" className='bg-blue-600 w-full p-3 text-white uppercase font-bold hover:bg-blue-700 cursor-pointer transition-colors rounded-md' value={paciente.id ? 'Editar Paciente' : 'Agregar Paciente'} />
      </form>
    </div>
  )
}
